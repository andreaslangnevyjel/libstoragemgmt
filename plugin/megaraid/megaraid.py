    ...
    def _wrapper(*args, **kwargs):
        try:
            ...
            raise LsmError(
                ...
                "Expected key missing from MegaRAID storcli output: {}".format(
                    key_error,
                ),
            )
...
def _lsm_raid_type_to_mega(lsm_raid_type) -> str:
    try:
        ...
        raise LsmError(
            ...
            "RAID type {:d} not supported".format(
                lsm_raid_type,
            ),
        )
...
class MegaRAID(IPlugin):
    ...
    def _storcli_exec(
        ...
        try:
            ...
            if os_error.errno == errno.ENOENT:
                raise LsmError(
                    ErrorNumber.INVALID_ARGUMENT,
                    "storcli binary '{}' is not exist or executable.".format(
                        self._storcli_bin,
                    ),
                )
            else:
        ...
        if flag_json:
            ...
            if len(ctrl_output) != 1:
                raise LsmError(
                    ...
                    "storcli: {}".format(str(output_dict)),
                )
    ...
    def _ctrl_count(self) -> int:
        ...
        if ctrl_count < 1:
            raise LsmError(
                ErrorNumber.NOT_FOUND_SYSTEM,
                "No MegaRAID controller detected by {}".format(
                    self._storcli_bin,
                ),
            )
        return ctrl_count
    ...
    def systems(self, flags=Client.FLAG_RSVD):
        ...
        for ctrl_num in range(self._ctrl_count()):
            ...
            plugin_data = "/c{:d}".format(ctrl_num)
    ...
    def _dg_top_to_lsm_pool(self, dg_top, free_space_list, ctrl_num,
        ...
        plugin_data = "/c{:d}/d{}".format(ctrl_num, dg_top["DG"])
    ...
    def _vd_to_lsm_vol(vd_id, dg_id, sys_id, vd_basic_info, vd_pd_info_list,
        ...
        vol_id = "{}:VD{:d}".format(sys_id, vd_id)
        name = "VD {:d}".format(vd_id)
        ...
        for ctrl_num in range(self._ctrl_count()):
            ...
            for key_name in list(vol_show_output.keys()):
                if key_name.startswith("/c"):
                    ...
                    dg_id = int(dg_id)
                    vd_id = int(vd_id)
                    vd_pd_info_list = vol_show_output["PDs for VD {:d}".format(vd_id)]
    ...
    def volume_raid_create(self, name, raid_type, disks, strip_size,
        ...
        if dg_id == "-":
            raise LsmError(
                ...
                "but RAID is not created: {}".fomrat(list(dg_show_output.items())),
            )
        else:
            dg_id = int(dg_id)
