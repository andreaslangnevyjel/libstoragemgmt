...
from .utils import cmd_exec, ExecError
...
def _handle_errors(method: Callable) -> Callable:
    def _wrapper(*args, **kwargs):
        try:
            ...
            raise LsmError(
                ...
                f"Expected key missing from MegaRAID storcli output: {key_error!r}",
            )
            ...
            raise LsmError(
                ErrorNumber.PLUGIN_BUG,
                f"Got unexpected error {common_error!r}",
            )

    return _wrapper


def _blk_count_of(mega_disk_size) -> int:
    blk_count_regex = re.compile(r"(0x[0-9a-f]+) Sectors")
...
def _disk_type_of(disk_show_basic_dict: Dict) -> int:
    """
...
def _disk_status_of(
    disk_show_basic_dict: Dict,
    disk_show_stat_dict: Dict,
) -> int:
...
def _mega_size_to_lsm(mega_size) -> int:
    """
    ...
    if re_match:
        ...
        raise LsmError(
            ErrorNumber.PLUGIN_BUG,
            f"_mega_size_to_lsm(): Got unexpected LSI size string {mega_size!s}",
        )
...
def _pool_status_of(dg_top: Dict) -> int:
    """
    Return status
    """
...
def _pool_id_of(dg_id, sys_id) -> str:
...
def _mega_raid_type_to_lsm(vd_basic_info, vd_prop_info) -> int:
    ...
    if raid_type == Volume.RAID_TYPE_RAID1 and int(vd_prop_info["Number of Drives Per Span"]) >= 4:
        raid_type = Volume.RAID_TYPE_RAID10

    return raid_type


def _lsm_raid_type_to_mega(lsm_raid_type) -> str:
    try:
        return _LSM_RAID_TYPE_CONV[lsm_raid_type]
    except KeyError:
        raise LsmError(
            ErrorNumber.NO_SUPPORT,
            f"RAID type {lsm_raid_type:d} not supported",
        )


def _disk_rpm_of(disk_show_basic_dict) -> int:
    disk_media = disk_show_basic_dict["Med"]
    if disk_media == "HDD":
...
def _disk_link_type_of(disk_show_basic_dict) -> int:
    """
    Return the 'Drive /c0/e64/s0' entry of '/c0/e64/s0 show all'
    """
    disk_interface = disk_show_basic_dict["Intf"]
...
def _fix_bbu_cv_output(output: List[Dict]) -> Dict:
    """
...
def _mega_cv_to_lsm(sys_id, cv_show_all_output) -> Battery:
...
def _vd_path_of_lsm_vol(lsm_vol) -> str:
...
class MegaRAID(IPlugin):
    ...
    @_handle_errors
    def capabilities(self, system, flags=Client.FLAG_RSVD) -> Capabilities:
    ...
    def _storcli_exec(
        ...
        try:
            cmd_result = cmd_exec(storcli_cmds)
        except OSError as os_error:
            if os_error.errno == errno.ENOENT:
                raise LsmError(
                    ErrorNumber.INVALID_ARGUMENT,
                    f"storcli binary '{self._storcli_bin}' is not exist or executable.",
                )
            else:
                raise

        output = re.sub(r"[^\x20-\x7e]", " ", cmd_result.stdout)
        ...
        if flag_json:
            ...
            if len(ctrl_output) != 1:
                raise LsmError(
                    ErrorNumber.PLUGIN_BUG,
                    f"_storcli_exec(): Unexpected output from MegaRAID storcli: {output_dict!s}",
                )
    ...
    def _ctrl_count(self) -> int:
        ...
        if ctrl_count < 1:
            raise LsmError(
                ErrorNumber.NOT_FOUND_SYSTEM,
                f"No MegaRAID controller detected by {self._storcli_bin}",
            )
        return ctrl_count
    ...
    def _lsm_status_of_ctrl(ctrl_show_all_output) -> Tuple:
        ...
        if lsi_status_info["Controller Status"] == "Optimal":
            ...
            status_info = "{}: ".format(
                lsi_status_info["Controller Status"],
            )
    ...
    def _sys_id_of_ctrl_num(self, ctrl_num: int, ctrl_show_all_output=None):
        if ctrl_show_all_output is None:
            return self._storcli_exec(f"/c{ctrl_num:d} show")["Serial Number"]
        else:
    ...
    def pools(
        ...
        for ctrl_num in range(self._ctrl_count()):
            cc_vd_ids = []
            cc_dg_ids = []
            dg_show_output = self._storcli_exec(f"/c{ctrl_num:d}/dall show all")
            consist_check_output = self._storcli_exec(f"/c{ctrl_num:d}/vall show cc")
    ...
    def volumes(self, search_key=None, search_value=None,
        ...
        lsm_vols = []
        for ctrl_num in range(self._ctrl_count()):
            vol_show_output = self._storcli_exec(f"/c{ctrl_num:d}/vall show all")
            sys_id = self._sys_id_of_ctrl_num(ctrl_num)
    ...
    def volume_raid_info(self, volume, flags=Client.FLAG_RSVD) -> List:
        ...
        if raid_type == Volume.RAID_TYPE_RAID0:
            ...
            raise LsmError(
                ErrorNumber.PLUGIN_BUG,
                "volume_raid_info(): Got unexpected RAID type: {}".format(
                    vd_basic_info["TYPE"],
                ),
            )
    ...
    def pool_member_info(self, pool, flags=Client.FLAG_RSVD):
        ...
        try:
            ...
            try:
                ...
                detail_error = json_output["Controllers"][0]["Command Status"]["Detailed Status"]
            except Exception:
                raise exec_error
        ...
        for dg_disk_info in dg_show_all_output["DG Drive LIST"]:
            cur_lsi_disk_id = f"{ctrl_num}:{dg_disk_info['EID:Slt']}"
    ...
    def volume_raid_create(self, name, raid_type, disks, strip_size,
        ...
        name = re.sub(r"[^0-9a-zA-Z_\-]+", "", name)[:15]

        if enclosure_str == " ":
            drives_str = "drives={}".format(",".join(slot_nums))
        else:
            drives_str = "drives={}:{}".format(enclosure_str, ",".join(slot_nums))
        ...
        if raid_type == Volume.RAID_TYPE_RAID10 or \
           ...
           raid_type == Volume.RAID_TYPE_RAID60:
            cmds.append("pdperarray={:d}".format(int(int_div(len(disks), 2))))

        if strip_size != Volume.VCR_STRIP_SIZE_DEFAULT:
            cmds.append("strip={:d}".format(int(int_div(strip_size, 1024))))
        ...
        dg_show_output = self._storcli_exec(
            ["/c{}/e{}/s{}".format(tuple(disks[0].plugin_data.split(":"))), "show"]
        )
        ...
        if dg_id == "-":
            raise LsmError(
                ...
                "but RAID is not created: {}".format(list(dg_show_output.items())),
            )
        else:
            dg_id = int(dg_id)
    ...
    def volume_cache_info(self, volume, flags=Client.FLAG_RSVD):
        ...
        vd_basic_info = vol_show_output[vd_path][0]
        vd_id = int(vd_basic_info["DG/VD"].split("/")[-1])
        vd_prop_info = vol_show_output["VD{:d} Properties".format(vd_id)]
        ...
        if lsi_cache_setting.endswith("D"):
            ...
            raise LsmError(
                ErrorNumber.PLUGIN_BUG,
                "Unknown I/O type {} for volume {}".format(
                    lsi_cache_setting,
                    vd_path,
                ),
            )
        ...
        if lsi_disk_cache_setting == "Disabled":
            ...
            raise LsmError(
                ErrorNumber.PLUGIN_BUG,
                "Unknown disk cache policy '{}' for volume {}".format(
                    lsi_disk_cache_setting,
                    vd_path,
                ),
            )
    ...
    @_handle_errors
    def volume_read_cache_policy_update(
        self,
        volume,
        rcp,
        flags=Client.FLAG_RSVD,
